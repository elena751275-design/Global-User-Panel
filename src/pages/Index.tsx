...
const Index = () => {
  ...
  return (
    <div className="min-h-screen pb-24 bg-background">
      <div className="relative px-4 py-4 overflow-hidden" style={{ background: "var(--gradient-header)" }}>
        ...
        <div className="grid grid-cols-4 gap-3 rounded-2xl glass p-4">
          {quickActions.map((item) => (
            <button key={item.label} className="flex flex-col items-center gap-2.5 group">
              <div className="h-14 w-14 rounded-xl bg-primary/15 border border-primary/30 flex items-center justify-center group-hover:bg-primary/25 transition-all shadow-[0_0_18px_-8px_hsl(var(--primary)/0.3)]">
      ...
      <div className="mx-4 mt-4">
        <Card className="bg-card border border-border rounded-2xl shadow-[var(--shadow-tile)] overflow-hidden relative">
          ...
          <CardContent className="p-4">
            <div className={`flex items-center gap-3 transition-all duration-500 ${isAnimating ? "opacity-0 -translate-y-3" : "opacity-100 translate-y-0"}`}>
      ...
      {/* Platform Introduction - CONSISTENT SPACING */}
          ...
          {platformItems.map((item) => (
            <Card key={item.title} className="overflow-hidden border border-border bg-card rounded-2xl shadow-[var(--shadow-tile)] hover:border-primary/40 hover:shadow-[0_0_15px_-3px_rgba(184,134,11,0.2)] transition-all cursor-pointer">
              ...
              <div className="aspect-video w-full overflow-hidden bg-secondary">
                <img src={item.image} alt={item.title} className="w-full h-full object-cover grayscale-[40%] contrast-110" loading="lazy" />
              </div>
      ...
      {/* Trusted Partners - UNIFORM GRID */}
      <div className="px-4 mt-6 pb-4">
        <h2 className="text-base font-bold text-foreground mb-4 uppercase tracking-wide">Our Trusted Global Partners</h2>
        <div className="grid grid-cols-4 gap-2.5">
          {partners.map((partner) => (
            <div key={partner.name} className="aspect-square bg-card border border-border rounded-xl flex items-center justify-center p-2.5 shadow-[var(--shadow-tile)] hover:border-primary/40 transition-all">
              <img src={partner.logo} alt={`${partner.name} logo`} width={64} height={64} loading="lazy" className="max-h-full max-w-full object-contain" />
            </div>
          ))}
        </div>
      </div>
